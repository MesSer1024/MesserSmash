    ...
    class RegisterHighscoreCommand : Command {
        ...
        public RegisterHighscoreCommand(string playerName) : base(NAME) {
            ...
            foreach (var i in levelScore) {                
                totalScore += i.Score;
                totalkills += i.Kills;
                LocalHighscore.Instance.resetWithLevelInsertAndOutputToFile(playerName, i.Level, i.Score, i.Kills);
            }
            Logger.info("RegisterHighscoreCommand: {0} - {1} - {2}", playerName, totalScore, totalkills);
            LocalHighscore.Instance.resetWithTotalScoreAndInsertScoreAndOutputToFile(playerName, totalScore, totalkills);
        }
    }
}
