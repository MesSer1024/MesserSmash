...
namespace MesserSmash.Arenas {
    class Level2 : Arena {
        ...
        private const int ID_LEVEL2_MAX_ENEMIES_PER_WAVE = 6;
        private const float ID_LEVEL2_TIME_BETWEEN_WAVES = 4;
        private const float ID_LEVEL2_BETWEEN_EACH_UNIQUE_SPAWN_CD = 0.15f;
        ...
        public override void startLevel() {
            _timestampLastSpawnedWave = ID_LEVEL2_TIME_BETWEEN_WAVES - 3;
        ...
        protected override void custUpdate(float gametime) {
            ...
            if (_timestampLastSpawnedWave >= ID_LEVEL2_TIME_BETWEEN_WAVES) {
                _internalWaveTimer += gametime;
                if (_internalWaveTimer >= ID_LEVEL2_BETWEEN_EACH_UNIQUE_SPAWN_CD && _spawnCounter < ID_LEVEL2_MAX_ENEMIES_PER_WAVE) {
                    getRandomSpawnpoint().generateRangedEnemies(1);
                    _internalWaveTimer = 0;
                    _spawnCounter++;
                } else if (_spawnCounter >= ID_LEVEL2_MAX_ENEMIES_PER_WAVE) {
                    _spawnCounter = 0;
                    _internalWaveTimer = 0;
